...
```{r creating dataset}
allinstitutions <- read_excel("PercentageFinancialAid2000-2018.xls", 
                              skip = 1) %>%
  select(1:8, 13:16)
...
# Suppress warning here because some observations in academic year aren't 
# numeric, they're characters that I will eventually clean out, so the warning
...
characteristics_1516 <- read_excel("Student_Characteristics_Aid15-16.xls", 
                                   skip = 2)
suppressWarnings(expr)

# Suppressing another warning here because they're telling me the columns names 
# have been changed, not a big deal since I will be renaming these column names
# in the code below after I select:
...
demographic <- bind_rows(sex, race, fam_inc, marital, housing, age) %>%
  ...
  pivot_longer(cols = c(-characteristic, -data_type), names_to = "aid_type", 
               values_to = "amount") %>%
...
#Suppressing once again because R wants me to specify column types for all the
# 200+ columns,but I only need a select few, so specifying all of them would 
# take up too much irrelevant space since I won't be using more of the variables
# anywya.
...
```{r}
# Tests. I run tests of my plots here, here's an example of a line plot I ran
# that appears in my tabl Institutional Trends 2000-2018!
